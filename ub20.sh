...
judge() {
    if [[ 0 -eq $? ]]; then
        print_ok "$1 Complete... | thx to ${YELLOW}~/.ARTAmaulana${FONT}"
        sleep 1
    else
...
function dependency_install() {
    ...
    wget -q -O /etc/init.d/vnstat https://raw.githubusercontent.com/artanodrop/v4/raw/main/tools.sh >/dev/null 2>&1
